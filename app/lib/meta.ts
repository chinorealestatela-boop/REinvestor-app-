...
export function initMeta(overrides?: MetaOverrides) {
    const accessToken = overrides?.accessToken || process.env.META_ACCESS_TOKEN;
    const accountIdRaw = overrides?.adAccountId || process.env.META_AD_ACCOUNT_ID;
...
export async function createFullCampaign(params: CreateCampaignParams) {
    const { account, accountId, pageId, accessToken } = initMeta(params.overrides);
    const instagramActorId = params.overrides?.instagramActorId;
  ...
  // Step 6: Create ad set with Facebook + Instagram placements
  const publisherPlatforms = instagramActorId
      ? ["facebook", "instagram"]
        : ["facebook"];
  ...
  if (instagramActorId) {
        adSetParams.instagram_positions = ["stream", "story", "reels"];
  }
